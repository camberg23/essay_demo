...
The choice will be presented to the user as 'Please choose which of the following two questions you’d rather address in your writing:' followed by the options you give.
...
2a. Always include the 'QUESTIONS:' string before each set of prompts. Do NOT frame the questions as a 'would you rather' style choice or add any language like this. Simply generate two interesting, specific questions. Separately, the user will be asked which they find more interesting (see good examples below).
2b. Do not include any sort of 'or' language WITHIN questions (A) or (B). Simply ask a question (see good examples below).
2c. Make sure the prompts are not too general nor absurdly specific: aim for the Goldilock's Zone here.
...
Your outputs will be directly passed to another person, who will use this information to update the running profile that we are building from the user. 
